...
ensure_and_load_env_defaults() {
  local mobile_caddy_key_pattern
...
EOF
    err "[config] Created .env with default GNTL_MOBILE_USE_CADDY=1"
  elif ! grep -Eq "$mobile_caddy_key_pattern" "$ENV_FILE" 2>/dev/null; then
    printf '\nGNTL_MOBILE_USE_CADDY=1\n' >> "$ENV_FILE"
